...
def tall_building_finder()
    bbox_coordinates =  $local_authority_coordinates
    offset_amount = 0
    num_of_results = 100
    list_of_building_info = []

    while num_of_results == 100
        url = URI("https://api.os.uk/features/ngd/ofa/v1/collections/bld-fts-buildingpart/items?bbox=#{bbox_coordinates}&filter=absoluteheightmaximum%20%3E%2018.0&offset=#{offset_amount.to_s}&key=CkhkKSjbQD0FoWPnevbA2vdfsBAe1rv8")
