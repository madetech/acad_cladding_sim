...
def uprn_finder()
  ...
  first_25_buildings_ngd_info = full_list_of_ngd_info[0..24]  # Set number of results to display
  first_25_buildings_info_with_uprn = []  # Stores returned UPRNs
  
  # Calls the OS Linked Identifier API on each toid
  first_25_buildings_ngd_info.each do |toid_resstatus_height|
    ...
    begin
      ...
      first_25_buildings_info_with_uprn.append(["UPRN not available", toid_resstatus_height])
      next
    end
    first_25_buildings_info_with_uprn.append([uprn, toid_resstatus_height])
  end
  [first_25_buildings_info_with_uprn, full_list_of_ngd_info.length.to_s]
end
